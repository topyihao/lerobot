...
def process_single_episode(hdf5_path: str, episode_idx: int):
    ...
    with h5py.File(hdf5_path, 'r') as f:
        ...
        print(f"  Episode {episode_idx} - Actual length: {episode_length}/{num_timesteps}")
...
def convert_aloha_folder_to_lerobot(
    ...
    if not hdf5_files:
        raise ValueError(f"No HDF5 files found in {folder_path} matching pattern {file_pattern}")
    ...
    features = {
        "observation.state": {
            ...
            "names": [f"joint_{i}" for i in range(first_episode_data['qpos'].shape[-1])]
        },
        ...
        "action": {
            ...
            "names": [f"action_{i}" for i in range(first_episode_data['actions_data'].shape[-1])]
        },
    }
    ...
    for episode_idx, hdf5_file in enumerate(hdf5_files):
        ...
        for frame_idx in range(episode_length):
            ...
            if any(img is None for img in [cam_high_img, cam_left_wrist_img, cam_right_wrist_img]):
    ...
    print(f"\n=== Conversion Complete ===")
    print(f"Successfully converted {len(hdf5_files)} episodes with {total_frames} total frames!")
...
if __name__ == "__main__":
    ...
    convert_aloha_folder_to_lerobot(
        folder_path="/home/allied/Disk2/Yihao/aloha_data/aloha_clean_dish",
        repo_id="local/aloha_clean_dish",
        task_description="use left arm pick up the dish, use right arm pick up the sponge and clean the dish",
        output_dir="/home/allied/Disk2/Yihao/lerobot_dataset/aloha_clean_dish_v2",  # Different name
        fps=50,
        max_workers=4,
